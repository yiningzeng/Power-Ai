...
export interface ICloudFilePickerProps {
    ...
    onSubmit: (success: boolean, belongToProject: IProjectItem, copyList?: string[]) => void;
 ...
 */
export class CloudFileCopyPicker extends React.Component<ICloudFilePickerProps, ICloudFilePickerState> {
                ...
                <ModalBody>
                    <div>
                        ...
                        <InputGroup>
                            ...
                            <Input name="path" id="path" defaultValue="Power-Ftp" placeholder="共享的目录" onChange={(v) => {
                                this.setState({
                                    ...this.state,
                                    ...
                                    <Button style={{width: "100%"}} color="warning" onClick={async () => {
                                        ...
                                        pTimeout(aa, 10000, () => {
                                            ...
                                            if (val === "success") {
                                                ...
                                                // tslint:disable-next-line:max-line-length
                                                const fileFolder = await this.localFileSystem.copyRemoteContainer(defaultPath);
    ...
    private async ok() {
        this.draggableDialog.current.open();
        let num = 0;
        await this.state.copyList.mapAsync(async (val) => {
            num = num++;
        ...
        this.draggableDialog.current.change("拷贝完成",
            `已经全部拷贝完成!`, true, false);
        this.props.onSubmit(true, this.state.belongToProject, this.state.copyList);
    }
